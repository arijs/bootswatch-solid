...
export function classes(...c: (ClassArg | undefined)[]) {
	...
	const handleRecord = (r: ClassRec) =>
		Object.entries(r)
			.filter(([_k, v]) => v)
			.map(([k]) => handleString(k))
	const handleMono = (m: ClassMono) => ('string' === typeof m ? handleString(m) : handleRecord(m))
	const handleList = (l: ClassList) => l.forEach((x) => handleMono(x))
	c.forEach((x) => (Array.isArray(x) ? handleList(x) : x && handleMono(x)))
	return clist.join(' ')
}
