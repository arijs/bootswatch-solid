...
| 6 | cosmo | ✅ | 433/433 @ 0.000000. Full baseline regen first, then verified clean on the first pass — no emitter fix (VE already rendered correctly). |
...
| 9 | flatly | ✅ | 433/433 @ 0.000000. Full baseline regen first, then verified clean on the first pass — no emitter fix (VE already rendered correctly). |
| 10 | journal | ✅ | 433/433 @ 0.000000. No emitter fix — baselines regenerated (stale), VE already rendered correctly. |
| 11 | litera | ✅ | 433/433 @ 0.000000. Full baseline regen first. One general component fix: the heading examples render `<p class="h1">…</p>` but stamped only `clsH1`–`clsH6`, not the `paragraph` element contract — so litera's `p { font-family: Georgia,…,serif }` override never fired and headings rendered sans-serif. Added `paragraph` to `Heading.tsx` and `HeadingBadges.tsx`. Neutral for other themes (their `p{}` font = body font; `.h1` margin wins on specificity). `FullscreenModal` uses a real `<h5>`, not `<p>`, so unaffected. |
...
| 17 | pulse | ✅ | 433/433 @ 0.000000. Full baseline regen first, then verified clean on the first pass — no emitter fix (VE already rendered correctly). |
...
| 26 | yeti | ✅ | 433/433 @ 0.000000. Full baseline regen first. One general component fix: yeti's `.nav-pills .active { border: 1px solid }` maps to `${scope}${navPills} ${scope}${active}`, but `PillNav.tsx`'s active link stamped only `navLinkActive` (`.nav-link.active`), not the bare literal `active` contract — so the border never fired, the baseline's taller active pill shifted the whole nav row, and every item's text mismatched. Stamped `active` on the active pill link. Neutral for other themes (baseline active link always carries `.active`; they lack a `.nav-pills .active` rule). |
| 27 | zephyr | ✅ | 433/433 @ 0.000000. Full baseline regen first, then verified clean on the first pass — no emitter fix (VE already rendered correctly). |

**Done: 27/27 — all themes verified to zero. 🎉** T9 complete.
