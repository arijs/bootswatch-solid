...
- **F1 — bootstrap completo:** `scripts/build-package.mjs` compila **todas as ~33 famílias** do bootstrap + `scope` + `public-vars` + `contract.js` (~575 nomes hasheados), só com o plugin VE, identifiers determinísticos (`bsve_`), naming normalizado. `utilities` baked excluída (o caminho é o preset). **Verificação de pixel:** o harness oficial (`capture-leaf-screenshots.mjs`) roda o pixel-diff — no ambiente do Rafael (fontes batem com os baselines); no sandbox do agente o diff acusa só AA/fonte de texto (caixa/cor idênticas, confirmado). **Literal residual do PORT:** 24 declarações `:root` informativas no `global` (gray/breakpoints, majoritariamente não-referenciadas) + 2 refs de var de componente (`progress-height`, `breadcrumb-font-size`) — some quando o conversor do port hashear essas vars (sweep de port, separado). **Falta ainda:** `package.json`/exports.
