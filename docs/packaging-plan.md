...
- **F2 — Todos os temas:** estender o build de componentes aos 27 (o `pack-dist.mjs` já enumera temas — basta o `build-package` rodar `--all-themes`); validar granular por família. O `contract`/`preset`/`solid` são compartilhados (theme-agnostic); só entram mais pastas `themes/<tema>/`.
