...
- **F8 — Utilities por-tema + purge JIT próprio (PLANEJADO):** abandonar o preset UnoCSS theme-agnostic e tratar utilities como as demais famílias — CSS por-tema hasheado, fiel à regra de cada tema (assumindo 100% theme-dependente). A otimização (dropar utilities não usadas) vira um plugin Vite próprio de purge. Motivo: o preset assa valores do bootstrap padrão e diverge de todo utility custom por tema (text-bg-*, fw-*, fs-*/RFS, spacers). Breaking → 0.2.0. Design completo em `docs/utilities-plan.md`.
```
