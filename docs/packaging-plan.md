...
- **F4 — FEITO:** runtime `/solid` (`BootswatchProvider` + `useBootswatch` + `useUtility`/`u()`; `prefixClasses` puro testado). **Reatividade (21/07):** o `BootswatchProvider` passou a aceitar `scope`/`utilityPrefix` como valor fixo OU **accessor reativo** (o `scope` era capturado UMA vez → não servia p/ troca de tema em runtime, e o frontend DDSOFT teve de fazer um provider próprio). O contexto agora expõe `scope`/`utilityPrefix` como `Accessor<string>` + os helpers `cx(...)` (scope + contracts, reativo) e `u(...)` (utilities com prefixo), com atalhos `useScope`/`useCx`/`useUtility`. A carga do CSS do tema e a persistência ficam no app (specifiers de import variam por bundler); o provider só distribui o scope. Isso permite ao DDSOFT trocar o `useUi` custom por `useBootswatch()`. O preset ganhou um **extractor** que reconhece `u('...')` e entrega os tokens já prefixados ao UnoCSS (senão o JIT, que varre a fonte, não geraria a classe prefixada que o `u()` emite em runtime). Demo integrada em `examples/demo/` (Solid + UnoCSS+preset prefixo `bsu-` + CSS do `dist-pkg`), **compilada e renderizada no Chromium**: botão com scope+contract (VE hasheado) + `bsu-mb-3`/`bsu-me-2` (JIT, só as usadas) + fundo `rgb(13,110,253)` vindo da var pública **hasheada**. Descoberta (já resolvida): restavam `--bs-*` literais no port (escala `--bs-gray-*`, `--bs-breakpoint-*`, uns rgb) — varridos no **sweep de port** da F1 (20/07); artefato hoje com **0 literal**.
...
- **F7 — Runtime JS do Bootstrap (PLANEJADO):** publicar o fork como `@arijs/bootstrap-runtime` + adicionar o entry `@arijs/bootswatch-ve/runtime` com os 8 bridges `ve-*`, p/ que os componentes JS (modal/offcanvas/collapse/carousel/tooltip/popover/toast/dropdown) funcionem contra o CSS hasheado num consumidor do npm. Hoje só funciona no monorepo (o pacote publicado não leva runtime JS). O DDSOFT **não** precisa (reimplementa interatividade em Solid). Não bloqueia a 0.1.0 — é incremento posterior (0.2.0). Design completo em `docs/runtime-plan.md`.
- **F8 — Utilities por-tema + entradas por-família + purge por imports (PLANEJADO):** abandonar o preset UnoCSS; utilities viram CSS por-tema hasheado (fiel a cada tema, 100% theme-dependente). Cada classe — inclusive estados (show/active/fade) — é importada da sua família (`@arijs/bootswatch-ve/buttons`, `/utilities`, `/state`), sem `u()`/prefixo, consumida por `cx`. O plugin Vite faz purge de TODAS as famílias analisando os imports estáticos por família (o import = declaração de uso, resolvendo classes dinâmicas sem safelist; só scopes/vars são sempre-mantidos). Breaking → 0.2.0. Design em `docs/utilities-plan.md`.
```
