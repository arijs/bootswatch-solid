...
- **F5 — CI + publish — FEITO (workflows) / pendente (1ª publicação):** dois workflows do GitHub Actions.
  ...
  - `.github/workflows/publish.yml` (ao publicar um Release `vX.Y.Z`, ou manual): roda os gates, monta `package/` com a versão da tag (`BSVE_VERSION`) e faz `npm publish` de dentro. **Pré-requisitos p/ a 1ª publicação:** secret `NPM_TOKEN` (token de automação com publish no escopo `@arijs`); `access: public` já está no `package.json` gerado. Scripts novos no `package.json` da raiz: `pkg:generate|build|pack|probe|ci`.
