...
- **F8 — Utilities por-tema + purge JIT de TODAS as famílias (PLANEJADO):** abandonar o preset UnoCSS e tratar utilities como as demais famílias — CSS por-tema hasheado, fiel a cada tema (assumindo 100% theme-dependente), consumido **só via `cx(C.x)`** (elimina o `u()`/prefixo). O purge vira um plugin Vite próprio que enxuga **todas as famílias** (componentes + utilities), tratando classes dinâmicas (estados `show`/`active`, scopes, vars) por safelist. Motivo: o preset assa valores do bootstrap padrão e diverge de todo utility custom por tema. Breaking → 0.2.0. Design em `docs/utilities-plan.md`.
```
