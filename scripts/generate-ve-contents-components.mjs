...
function transformSourceComponent(source, targetFile, tokenToVeClass) {
	...
	let transformed = source.replace(
		/^import\s+type\s+\{\s*Component\s*\}\s+from\s+'solid-js'\s*\n?/m,
		'',
	)
	...
	const preservedImports = oldImports.filter((line) => !line.includes("from 'solid-js'"))
