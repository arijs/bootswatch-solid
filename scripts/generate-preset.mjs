...
const _BP_PX_TO_NAME = { 576: 'sm', 768: 'md', 992: 'lg', 1200: 'xl', 1400: 'xxl' }
...
const declsOf = (rule) =>
	Object.fromEntries(
		rule.declarations.filter((d) => d.type === 'declaration').map((d) => [d.property, d.value]),
	)
const declsEqual = (a, b) => JSON.stringify(a) === JSON.stringify(b)
const stripBp = (name) => name.replace(/-(sm|md|lg|xl|xxl)-/, '-')

async function main() {
	const { css: rawCss } = sass.compile(ENTRY, {
		style: 'expanded',
		quietDeps: true,
	...
	const overrideBrowserslist = readFileSync(
		path.join(ROOT, 'bootstrap-fork', '.browserslistrc'),
		'utf8',
	)
		.split('\n')
		.map((l) => l.replace(/#.*/, '').trim())
		.filter(Boolean)
	const { css } = await postcss([autoprefixer({ overrideBrowserslist })]).process(rawCss, {
		from: undefined,
	})
	const ast = parse(css)
	...
	console.log(
		`variantes responsivas checadas: ${responsiveChecked} (mismatches: ${mismatches.length})`,
	)
	...
	console.log(
		`overrides mesmo-nome em @media (gap: RFS fs-*/reduced-motion): ${[...nonInfixOverrides].length}`,
	)
	if (nonInfixOverrides.size)
		console.log('   ex.:', [...nonInfixOverrides].slice(0, 8).join('  '))
	...
	console.log(
		`--bs-* NÃO mapeadas (deve ser 0): ${leftover.size}${leftover.size ? ` → ${[...leftover].join(', ')}` : ' ✔'}`,
	)
...
function loadPublicVarMap() {
	const names = JSON.parse(
		readFileSync(path.join(ROOT, 'preset', 'public-vars.names.json'), 'utf8'),
	)
	const hashes = JSON.parse(
		readFileSync(path.join(ROOT, 'preset', 'public-vars.hash.json'), 'utf8'),
	)
	const map = {}
