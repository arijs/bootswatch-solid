...
const VE_BUTTONS_PAGE_TSX = path.join(
	ROOT,
	've-project',
	'src',
	'components',
...
const VE_BUTTONS_LINKS_TS = path.join(
	ROOT,
	've-project',
	'src',
	'components',
...
function transformSourceComponent(source, targetFile) {
	...
	if (mapped.length > 0) {
		veImports.push(`import { ${mapped.join(', ')} } from '${generatedButtonsImport}'`)
	}
...
async function main() {
	...
	const buttonLinksContent = `export const buttonLinks = [\n${buttonRoutes
		.map(
			(entry) =>
				`\t{ href: '${entry.routePath}', label: '${routeToLabel(entry.routePath)}' },`,
		)
		.join('\n')}\n] as const\n`
