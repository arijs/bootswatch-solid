...
function renderJsxNode(sourceFile, node, renderClassAttribute, indentLevel) {
  ...
  if (ts.isJsxElement(node)) {
    const attributes = renderAttributes(sourceFile, node.openingElement.attributes, renderClassAttribute);
    ...
    const children = node.children
      .map((child) => renderJsxNode(sourceFile, child, renderClassAttribute, childIndentLevel))
      .filter(Boolean);
