...
const ButtonGroupExample: Component = () => {
	...
	return (
		<div
			class={`bd-example ${buttonCls.bsTheme} ${buttonCls.body} ${buttonCls.containerFluid}`}
		>
			<div class={groupCls.btnToolbar} role="toolbar" aria-label="Toolbar with button groups">
				<fieldset
					class={`${groupCls.btnGroup} ${groupCls.marginEnd2}`}
					aria-label="First group"
				>
					<button
						type="button"
						class={`${buttonCls.btn} ${buttonCls.btnSecondary} ${groupCls.btnGroupButton} ${groupCls.btnGroupInteractive}`}
					>
						1
					</button>
					<button
